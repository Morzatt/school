...
<div class="relative flex flex-col justify-between min-h-[14rem] overflow-hidden transition-all duration-300 
    ...
    <div> <!-- Content wrapper -->
        ...
        <div class="mb-5 text-sm p-2 bg-base-100/50 rounded-lg flex items-center gap-2 shadow-sm">
            ...
            <span class="truncate">Docente: <span class="font-semibold {grado.nivel === 'Inicial' ? 'text-primary' : 'text-accent'}">{grado.profesor ? `${grado.nombre_profesor} ${grado.apellido_profesor}` : "No Asignado"}</span></span>
                </div>
