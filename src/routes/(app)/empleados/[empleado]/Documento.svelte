<script lang="ts">
    import type { DocumentoAlumno, Empleado, TiposDocumentos } from "$lib/database/types";
    ...
    let { path, tipo_documento, empleado, visible }: {
                ...
                <form method="post" action="?/updateFotoPerfil" use:enhance={() => {
                    ...
                    <div class='w-full min-h-12 mt-1 flex items-center justify-start gap-6'>
                        <label class="btn btn-square btn-primary tooltip tooltip-top" data-tip="Actualizar Documento">
                            ...
                            <input type="hidden" name="cedula_empleado" value="{empleado.cedula}">
                        ...
                        onclick={() => {
                            if (path) {
                                downloadFile(`/downloads/0000?type=image&path=${path}`, `${tipo_documento}_${empleado.cedula}`)
                            }
                        }}>
